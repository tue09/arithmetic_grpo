#!/usr/bin/env bash
# Qwen3-4B-Instruct GRPO run on DAPO-Math-17k with the local 4-GPU math setup.
# This keeps the current math hyperparameters and switches the dataset/reward
# wiring to the DAPO parquet format.
...
RUN_TAG="${RUN_TAG:-qwen3_4b_instruct_grpo_dapo_math_17k_4gpu_clip_ratio_high_0.24}"
...
Then rerun:
  bash $PROJECT_DIR/run_qwen3_4b_instruct_grpo_dapo_math_17k_4gpu.sh
EOF
    exit 1
fi
