...
DATA_SETTING="${DATA_SETTING:-gsm8k_math}"
...
DEFAULT_TRAIN_DATA_PRESET="gsm8k_math"
DEFAULT_VAL_DATA_PRESET="gsm8k_math"
...
case "$DATA_SETTING" in
    ...
    gsm8k_math_aime24)
        ...
        ;;
    gsm8k_math_aime25)
        DEFAULT_VAL_DATA_PRESET="aime25"
        ;;
    gsm8k_math_aime24_aime25 | gsm8k_math_aime25_aime24)
        DEFAULT_VAL_DATA_PRESET="aime24_aime25"
        ;;
    aime24)
        DEFAULT_TRAIN_DATA_PRESET="aime24"
        DEFAULT_VAL_DATA_PRESET="aime24"
        ;;
    aime25)
        DEFAULT_TRAIN_DATA_PRESET="aime25"
        DEFAULT_VAL_DATA_PRESET="aime25"
        ;;
    aime24_aime25 | aime25_aime24)
        DEFAULT_TRAIN_DATA_PRESET="aime24_aime25"
        DEFAULT_VAL_DATA_PRESET="aime24_aime25"
        ;;
    *)
        echo "Unsupported DATA_SETTING: $DATA_SETTING" >&2
        echo "Supported values: gsm8k_math, gsm8k_math_aime24, gsm8k_math_aime25, gsm8k_math_aime24_aime25, aime24, aime25, aime24_aime25" >&2
        exit 1
        ;;
esac
...
resolve_data_preset() {
    ...
    case "$preset" in
        gsm8k_math)
            if [ "$split" = "train" ]; then
                printf '%s\n' "$GSM8K_TRAIN" "$MATH_TRAIN"
            else
                printf '%s\n' "$GSM8K_TEST" "$MATH_TEST"
            fi
            ;;
        aime24)
            printf '%s\n' "$AIME24_FILE"
...
if [ -z "$TRAIN_FILES_OVERRIDE" ]; then
    TRAIN_PATHS_RAW="$(resolve_data_preset train "$TRAIN_DATA_PRESET")"
...
if [ -z "$VAL_FILES_OVERRIDE" ]; then
    VAL_PATHS_RAW="$(resolve_data_preset val "$VAL_DATA_PRESET")"
...
RUN_TAG="${RUN_TAG:-qwen2_5_3b_grpo_${TRAIN_DATA_PRESET}_to_${VAL_DATA_PRESET}}"
export WANDB_PROJECT="${WANDB_PROJECT:-verl_qwen2_5_3b_grpo}"
...
if [ "$missing" -ne 0 ]; then
    ...
    if [ "$missing_gsm8k" -eq 1 ] || [ "$missing_math" -eq 1 ]; then
        cat >&2 <<EOF
Create the GSM8K / MATH datasets from the repo root with:
...
EOF
    ...
    if [ "$missing_aime24" -eq 1 ] || [ "$missing_aime25" -eq 1 ]; then
        cat >&2 <<EOF
Create the AIME datasets from the repo root with:
  mkdir -p "$AIME_DIR"
...
Then rerun, for example:
  DATA_SETTING=gsm8k_math_aime24 bash $PROJECT_DIR/run_qwen2-5_3b_grpo_math_4gpu.sh
  DATA_SETTING=aime25 bash $PROJECT_DIR/run_qwen2-5_3b_grpo_math_4gpu.sh
EOF
    exit 1
fi
