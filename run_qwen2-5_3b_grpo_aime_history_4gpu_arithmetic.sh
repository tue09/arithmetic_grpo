#!/usr/bin/env bash
# DATA_SETTING=history_to_aime25 bash run_qwen2-5_3b_grpo_aime_history_4gpu_arithmetic.sh
set -euo pipefail
set -x
...
case "$DATA_SETTING" in
    history_to_aime24)
        ;;
    history_to_aime25)
        DEFAULT_TRAIN_DATA_PRESET="aime_history_1983_2024"
...
RUN_TAG="${RUN_TAG:-qwen2_5_3b_grpo_${TRAIN_DATA_PRESET}_to_${VAL_DATA_PRESET}_arithmetic}"
export WANDB_PROJECT="${WANDB_PROJECT:-verl_qwen2_5_3b_grpo_aime_history}"
...
for required_path in "${required_paths[@]}"; do
    if [ ! -e "$required_path" ]; then
        ...
        case "$required_path" in
            "$AIME_HISTORY_1983_2023" | "$AIME_HISTORY_1983_2024" | "$AIME_HISTORY_2024")
                missing_history=1
                ;;
            "$AIME24_FILE" | "$AIME25_FILE")
                missing_eval=1
...
Then rerun, for example:
  DATA_SETTING=history_to_aime24 bash $PROJECT_DIR/run_qwen2-5_3b_grpo_aime_history_4gpu_arithmetic.sh
  DATA_SETTING=history_to_aime25 bash $PROJECT_DIR/run_qwen2-5_3b_grpo_aime_history_4gpu_arithmetic.sh
EOF
    exit 1
fi
