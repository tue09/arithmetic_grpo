#!/usr/bin/env bash
# DATA_SETTING=history_to_aime25 bash run_qwen3_4b_grpo_aime_history_4gpu.sh
set -euo pipefail
set -x
...
RUN_TAG="${RUN_TAG:-qwen3_4b_grpo_${TRAIN_DATA_PRESET}_to_${VAL_DATA_PRESET}}"
...
Then rerun, for example:
  DATA_SETTING=history_to_aime24 bash $PROJECT_DIR/run_qwen3_4b_grpo_aime_history_4gpu.sh
  DATA_SETTING=history_to_aime25 bash $PROJECT_DIR/run_qwen3_4b_grpo_aime_history_4gpu.sh
EOF
    exit 1
fi
