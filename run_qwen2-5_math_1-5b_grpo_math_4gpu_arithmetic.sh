#!/usr/bin/env bash
# Paper-aligned MATH arithmetic-GRPO baseline for Qwen2.5-Math-1.5B.
# Source: https://arxiv.org/pdf/2601.20614, Section F.1 / Table 9.
# Note: the local MATH parquet already places the reasoning instruction in the
# user message, so this script matches the paper's training hyperparameters but
# does not rewrite the dataset into a separate system prompt format.
...
export WANDB_PROJECT="${WANDB_PROJECT:-verl_qwen2_5_math_1_5b_grpo_math_paper}"
...
MODEL_PATH="${MODEL_PATH:-/mnt/data/safetyCode/model_hub/Qwen/Qwen2.5-Math-1.5B}"
...
RUN_TAG="${RUN_TAG:-qwen2_5_math_1_5b_grpo_math_paper_4gpu_arithmetic}"
...
missing=0
for required_path in "$MODEL_PATH" "$MATH_TRAIN" "$MATH_TEST"; do
...
Then rerun:
  bash $PROJECT_DIR/run_qwen2-5_math_1-5b_grpo_math_4gpu_arithmetic.sh
EOF
    exit 1
fi
...
"$PYTHON_BIN" -m verl.trainer.main_ppo \
    ...
    actor_rollout_ref.rollout.gpu_memory_utilization=${VLLM_GPU_MEMORY_UTILIZATION:-0.6} \
    actor_rollout_ref.rollout.temperature=${ROLLOUT_TEMPERATURE:-1.0} \
    actor_rollout_ref.rollout.top_p=${ROLLOUT_TOP_P:-1.0} \
    actor_rollout_ref.rollout.max_model_len=${MAX_MODEL_LEN:-1536} \
    ...
    trainer.total_training_steps=${TOTAL_TRAINING_STEPS:-1200} \
    "$@"
